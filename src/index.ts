...
} from "./ext-dependencies/balanceParser";
...
export class TxData {
  ...
  private ParseOptions(options?: TxDataOptions): void {
    if (typeof options === "object" && options !== null) {
      if (
        typeof options.EndpointTimeoutMs === "number" &&
    ...
    const minTimeoutMs =
      this.ConnectionAndQueryTimeoutMs * (this.Endpoints.length + 1);
    if (this.LookupTimeoutMs < minTimeoutMs) {
      this.LookupTimeoutMs = minTimeoutMs;
      logInvalid(
        "Overall timeout updated to min. endpoint * perEndpointTO seconds:",
        this.LookupTimeoutMs / 1000
      );
    }
  }
  ...
  public async get(
    ...
    const getPromise: Promise<ResolvedLookup> = new Promise(
      async (resolve, reject) => {
        ...
        const cleanup = () => {
          ...
          this.EventBus.listeners("tx." + TxHash).forEach((l) =>
            this.EventBus.removeListener("tx." + TxHash, l as any)
          );
        };
        ...
        const ResolveFormatted = (
          ...
          const finish = (customEventResult?: Tx, customHost?: string) => {
            ...
            const result = this.FormatResult(
              customEventResult ? customEventResult : eventResult
            );
            const balanceChanges =
              typeof (result as TxResult).meta !== "undefined"
            ...
            resolve({
              result: customEventResult?.result
                ? {
                    ...
                    ledger_index: (customEventResult.result as any)
                      ?.ledger_index,
          ...
          if (
            (eventResult as TxNotFound)?.error === "txnNotFound" &&
            WaitForSeconds > 0
          ) {
            // Not found
            if (!WaitMode) {
              log(
                "TX not found on ledger, could still arrive, wait for # sec.:",
                WaitForSeconds,
                TxHash
              );
        ...
        const onTx = (r: EmittedTxResult) => {
          if (
            ...
            ((this.FormatResult(r.result) as any)?.meta ||
              (this.FormatResult(r.result) as any)?.error)
          ) {
        ...
        for await (const val of this.ConnectAndQuery(TxHash)) {
          ...
          if (typeof val.socket !== "boolean") {
            if (
              ...
              ((this.FormatResult(val.result) as any)?.meta ||
                (this.FormatResult(val.result) as any)?.error)
            ) {
            ...
            if (val.url !== "") {
              logResolve(
                `reason @ ${val.url} =`,
                String(val?.resolveReason).toUpperCase()
              );
            } else {
  ...
  private async Connect(index = 0): Promise<WebSocket> {
    if (
      typeof this.ReadyConnections[index] === "undefined" &&
      ...
      const socket: WebSocket =
        typeof (global as any)?.MockedWebSocket !== "undefined" &&
      ...
      this.ReadyConnections[index] = new Promise((resolve) => {
        ...
        socket.onmessage = async (m) => {
          try {
            ...
            if (socketMeta.ready) {
              if (
                typeof (response as any)?.transaction?.validated ===
                  "undefined" ||
              ...
              if (
                typeof response?.result?.info?.complete_ledgers !== "undefined"
              ) {
                socketMeta.ready = true;
                const ledgerString = String(
                  response?.result?.info?.complete_ledgers || ""
                );
                const isFullHistory =
                  ledgerString.split(",").length < 2 &&
  ...
  private async *ConnectAndQuery(
    txHash: string
  ): AsyncGenerator<ConnectAndQueryResponse, void, unknown> {
  ...
  private async QueryConnection(
    ...
    return new Promise(async (resolve) => {
      if (connection.readyState === connection.OPEN) {
        connection.send(
          JSON.stringify({ id, command: "tx", transaction: TxHash })
        );
